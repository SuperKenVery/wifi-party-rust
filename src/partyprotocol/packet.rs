...
use rkyv::{
    ser::{
        serializers::{AllocSerializer, BufferSerializer},
        Serializer,
    },
    vec::ArchivedVec,
...
impl Packet {
    ...
    pub fn encode(&self) -> Result<AlignedVec, Box<dyn Error>> {
        match self {
            ...
            Packet::Received(_) => Err("Sending a received packet".into()),
        }
    }
...
impl OwnedArchivedPacket {
        ...
        match archive_result {
            Ok(archive) => {
                ...
                if archive.header.identifier != ['w', 'p', 'p', '\0'] {
                    ...
                    Ok(OwnedArchivedPacket { buffer: buf })
                }
            }
            Err(e) => Err(e.into()),
        }
    ...
    fn get_channel(&self) -> u32 {
        let archive = unsafe { rkyv::archived_root::<ConstructedPacket>(&self.buffer) };
